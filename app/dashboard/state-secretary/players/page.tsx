...
export default function StateSecretaryPlayersPage() {
  ...
  const exportToCSV = () => {
    const headers = ["Name", "Father's Name", "Email", "Mobile", "District", "Address", "Payment Status", "Registration Date"]
    ...
    a.href = url
    a.download = `${stateName}-players-${new Date().toISOString().split("T")[0]}.csv`
    a.click()
  }
  ...
  return (
    <div className="p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-foreground">
            {stateName || "State"} Players
          </h1>
          <p className="text-muted-foreground">
            View all registered players from your state
          </p>
        </div>
      ...
      <Card className="bg-card border-border mb-6">
        <CardHeader>
          <CardTitle className="text-lg text-foreground">Search Players</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input
              placeholder="Search by name, email, mobile, or district..."
              value={searchTerm}
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              {players.length === 0
                ? "No players registered from your state yet"
